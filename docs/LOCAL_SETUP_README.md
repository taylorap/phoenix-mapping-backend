...
This document provides two paths:
- Quick setup for experienced users who already have tools installed.
- Full from-scratch setup for first-time users.

The goal is to run the Phoenix Field Mapping (DB-backed) Explainer backend locally and hit:
...
```
MAPPING_DB_URL=postgresql://USER:PASSWORD@HOST:PORT/field_mapping_tool
OPENAI_API_KEY=sk-...    # optional, only needed for Function explanations
PORT=3000               # optional
```
...
If you see that response, you are ready to use `/api/explain`.

---

## Full setup (from scratch / first-time users)
...
- Can obtain:
  ...
  - An OpenAI API key or approved internal proxy key (optional but recommended).
...
- The RESO spec or mapping does not have that standardName.
...
## Windows notes (quick setup)

These steps mirror the macOS flow but use Windows-friendly tools.
...
If you use PowerShell without curl installed, you can also open:
